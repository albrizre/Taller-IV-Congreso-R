...
source("Models/Poisson_inhom4.R")
...
monitors_pars <- c("lambda0","beta","lambda_int","gamma1","gamma2")
Sys.time()
mcmc.output <- nimbleMCMC(Poisson_inhom4, data = data, inits = inits, constants = constants,
                          monitors = monitors_pars, 
                          niter = 20000, nburnin = 10000, nchains = 2, thin = 10,
                          summary = TRUE, WAIC = TRUE)
Sys.time()
# save(mcmc.output,file="Outputs/Poisson_inhom4.rda")
...
df_plot=c()
gammas1=mcmc.output$samples$chain1[,grep("gamma1",colnames(mcmc.output$samples$chain1))]
gammas2=mcmc.output$samples$chain1[,grep("gamma2",colnames(mcmc.output$samples$chain1))]
for (t in seq(0,365,1)){
  temp_effect=exp(gammas1*sin((2*pi*t)/30)+gammas2*cos((2*pi*t)/30))
